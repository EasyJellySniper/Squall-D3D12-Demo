...
RayPayload TestAmbient(RayDesc ray, bool testOcclusion)
{
    ...
    if (testOcclusion)
        TraceRay(_SceneAS, RAY_FLAG_CULL_NON_OPAQUE | RAY_FLAG_ACCEPT_FIRST_HIT_AND_END_SEARCH, ~0, 0, 1, 0, ray, payload);
    else
        TraceRay(_SceneAS, RAY_FLAG_CULL_NON_OPAQUE, ~0, 0, 1, 0, ray, payload);
