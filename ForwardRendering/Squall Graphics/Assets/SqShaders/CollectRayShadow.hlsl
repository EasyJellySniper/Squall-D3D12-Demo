...
float PenumbraFilter(float2 uv, int innerLoop)
{
    ...
    for (int i = -innerLoop; i <= innerLoop; i++)
    {
        for (int j = -innerLoop; j <= innerLoop; j++)
        {
            ...
            float4 shadowData = _RayShadow.SampleLevel(_SqSamplerTable[_LinearClampSampler], uv + _ScreenSize.zw * float2(i, j), 0);
...
{
    float center = _RayShadow.SampleLevel(_SqSamplerTable[_LinearClampSampler], uv, 0).r;
    [branch]
    ...
    for (int i = -innerLoop; i <= innerLoop; i++)
    {
        for (int j = -innerLoop; j <= innerLoop; j++)
        {
            ...
            atten += _RayShadow.SampleLevel(_SqSamplerTable[_LinearClampSampler], uv + float2(i, j) * _ScreenSize.zw * penumbra, 0).r * w;
            count += w;
        }
    }
...
{
    float depth = _DepthMap.SampleLevel(_SqSamplerTable[_LinearClampSampler], i.uv, 0).r;
    i.screenPos.z = depth;
